...
ban_button='{"inline_keyboard": [[{"text": "Ban", "callback_data": "ban_user '$PAM_USER' '$PAM_RHOST'"}]]}'
echo -e "📡New SSH login\n*🤖$PAM_USER* logged in on 🖥 *$HOSTNAME* at $date from $PAM_RHOST\n🌎Country:*$country*\n🏙 City=*$city*\n🕋Organisation=*$org*" > $msg
curl $sendmsg -d chat_id=$chat_id -d text="$(<$msg)" --data-urlencode "reply_markup=$ban_button"
